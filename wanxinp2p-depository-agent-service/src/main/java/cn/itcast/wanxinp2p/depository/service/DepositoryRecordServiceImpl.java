...
public class DepositoryRecordServiceImpl extends ServiceImpl<DepositoryRecordMapper, DepositoryRecord> implements DepositoryRecordService {
    ...
    public GatewayRequest createConsumer(ConsumerRequest consumerRequest) {
        ...
        System.out.println("到这里了DepositoryRecordServiceImpl.createConsumer depositoryurl" + configService.getDepositoryUrl());
        return gatewayRequest;
    }

    @Override
    public Boolean modifyRequestStatus(String requestNo, Integer requestsStatus) {
        return update(Wrappers.<DepositoryRecord>lambdaUpdate().eq(DepositoryRecord::getRequestNo, requestNo).set(DepositoryRecord::getRequestStatus, requestsStatus).set(DepositoryRecord::getConfirmDate, LocalDateTime.now()));
    }
    ...
    public DepositoryResponseDTO<DepositoryBaseResponse> createProject(ProjectDTO projectDTO) {
        ...
        if (null != depositoryResponseDTO) {
            return depositoryResponseDTO;
        }

        depositoryRecord = getEntityByRequestNo(depositoryRecord.getRequestNo());
    ...
    private DepositoryResponseDTO<DepositoryBaseResponse> handleIdempotent(DepositoryRecord depositoryRecord) {
        ...
        return JSON.parseObject(depositoryRecord.getResponseData(), new TypeReference<DepositoryResponseDTO<DepositoryBaseResponse>>() {
        });
    }
